...
{
    [kOS.Safe.Utilities.KOSNomenclature("ASteeringManager")]
    public class ASteeringManager : Structure, IDisposable, IFlightControlParameter
    {
        public static void Copy(ASteeringManager origin, ASteeringManager destination)
        {
            ...
            destination.omegaPI = PIDLoop.DeepCopy(origin.omegaPI);
        ...
        public bool Enabled
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    if (omegaWriter != null) omegaWriter.Dispose();
        ...
        private PIDLoop omegaPI = new PIDLoop(1, 0.1, 0, extraUnwind: true);
        private KSP.IO.TextWriter omegaWriter;
        ...
        private double lerpedOmegaEpsilon = 0.0;

        private double phi;
        private double maxOmega;
        private double tgtOmega;
        private Vector3d tgtOmegaAxis;
        ...
        public ASteeringManager(SharedObjects sharedObj) : this(sharedObj.Vessel)
        {
        }

        public ASteeringManager(Vessel vessel)
        {
        ...
        {
            AddSuffix("OmegaPID", new Suffix<PIDLoop>(() => omegaPI));
        ...
        private void Update(FlightCtrlState c)
        {
            ...
            if (Value == null)
            {
                SafeHouse.Logger.SuperVerbose("ASteeringManager.Update: Value is <null>");
                return;
            }
        ...
        private Direction GetDirectionFromValue(object value)
        {
            ...
            throw new Safe.Exceptions.KOSWrongControlValueTypeException(
                "ASTEERING", KOSNomenclature.GetKOSName(value.GetType()), "Direction, Vector, Maneuver Node, or special string \"KILL\"");
        }
        ...
        public void UpdateStateVectors()
        {
            if (Value == null)
            {
                ...
                shared.Logger.LogWarning("ASteeringManager target direction is null, skipping this update.");
                return;
            }
        ...
        {
            Quaternion qErr = vesselRotation.Inverse() * targetRot;
            qErr.ToAngleAxis(out float angleErr, out Vector3 axisErr);
            if (angleErr > 180f) angleErr = angleErr - 360f;
            if (Vector3d.Dot(axisErr, tgtOmegaAxis) < 0)
            {
                axisErr = -axisErr;
                angleErr = -angleErr;
            }
            phi = angleErr / RadToDeg;
            tgtOmegaAxis = axisErr.normalized;
            ...
            double maxPitchOmega = controlTorque.x * MaxStoppingTime / momentOfInertia.x;
            double maxYawOmega = controlTorque.z * MaxStoppingTime / momentOfInertia.z;
            double maxRollOmega = controlTorque.y * MaxStoppingTime / momentOfInertia.y;
            maxOmega = Math.Max(maxPitchOmega, Math.Max(maxYawOmega, maxRollOmega));

            double sampletime = shared.UpdateHandler.CurrentFixedTime;
            tgtOmega = omegaPI.Update(sampletime, -phi, 0, maxOmega, 0d);
            lerpedOmegaEpsilon = LerpEpsilon(tgtOmega, maxOmega, torquePIDEpsilonMin, torquePIDEpsilonMax);

            // Calculate target torque based on PID
            tgtPitchTorque = pitchPI.Update(sampletime, omega.x, -tgtOmega * tgtOmegaAxis.x, momentOfInertia.x, controlTorque.x, lerpedPitchEpsilon);
            tgtYawTorque = yawPI.Update(sampletime, omega.y, tgtOmega * tgtOmegaAxis.y, momentOfInertia.z, controlTorque.z, lerpedYawEpsilon);
            tgtRollTorque = rollPI.Update(sampletime, omega.z, -tgtOmega * tgtOmegaAxis.z, momentOfInertia.y, controlTorque.y, lerpedRollEpsilon);
        }
        ...
        public void UpdateVectorRenders()
        {
            ...
            if (ShowAngularVectors && enabled && !Vessel.ActionGroups[KSPActionGroup.SAS])
            {
                ...
                vTgtTorqueX.Vector = vesselTop * tgtOmega * (-tgtOmegaAxis.x) * RENDER_MULTIPLIER * 100f;
                vTgtTorqueX.Start = vesselForward * RENDER_MULTIPLIER;
                //vTgtTorqueX.SetLabel("tgtPitchOmega: " + tgtPitchOmega);
                vTgtTorqueY.Vector = vesselStarboard * tgtOmega * (-tgtOmegaAxis.z) * RENDER_MULTIPLIER * 100f;
                vTgtTorqueY.Start = vesselTop * RENDER_MULTIPLIER;
                //vTgtTorqueY.SetLabel("tgtRollOmega: " + tgtRollOmega);
                vTgtTorqueZ.Vector = vesselStarboard * tgtOmega * (tgtOmegaAxis.y) * RENDER_MULTIPLIER * 100f;
        ...
        public void PrintDebug()
        {
            ...
            shared.Screen.Print(string.Format("max omega: {0}", maxOmega * RadToDeg));
            shared.Screen.Print(string.Format("tgt omega: {0}", tgtOmega * RadToDeg));
        ...
        public void WriteCSVs()
        {
            if (omegaWriter == null)
            {
                omegaWriter = KSP.IO.File.AppendText<PIDLoop>(
                    string.Format(FILE_BASE_NAME, fileDateString, shared.Vessel.vesselName, "Rotation Rate"));
                omegaWriter.WriteLine("LastSampleTime,Error,ErrorSum,Output,Kp,Ki,Kd,MinOutput,MaxOutput");
            }
            ...
            {
                adjustTorqueWriter = KSP.IO.File.AppendText<ASteeringManager>(
            ...
            omegaWriter.WriteLine(omegaPI.ToCSVString());
        ...
        {
            ASteeringManager smOrigin = origin as ASteeringManager;
            if (smOrigin != null)
            {
                Copy(smOrigin, this);
            }
