...
PEGLand从一条合适的着陆轨道开始，引导飞船完成动力着陆的全过程。它包含三个制导段：

- 下降段 (descent phase)，从着陆轨道减速下降，瞄准着陆点附近。首先估计点火位置，在滑行到点火位置后，使用PEG算法迭代预测着陆点并更新控制参数，实现燃料最优的下降。**下降段中间没有无动力滑行段，程序将以几乎恒定的油门燃烧直至抵达地表附近**。如果航天器的引擎具有60%的节流能力，下降段是精确的。如果飞船引擎不具备足够的节流能力，为确保安全，制导程序将优先保证下降段终点的速度和高度，这将牺牲一部分着陆精度；
...
PEG算法不是魔法，它需要在物理约束下运行，这意味着你需要为着陆准备合适的初始轨道。在激活PEGLand制导并开始迭代计算之前，我强烈推荐你点击`Analyze Orbit`按钮快速分析一下你的着陆轨道，查看你是否具备着陆条件。它会给你两个指标，还有推荐的取值范围。在推荐的范围内，着陆过程的燃料需求基本都是一样的。但如果你偏离太远，会导致严重的燃料浪费，甚至制导发散。
